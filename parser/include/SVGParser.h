...
// The main struct, representing an svg element of the format
...
typedef struct {
    ...
    //it into the field. 
    //If the data exceeds there field length, truncate the data to fit the max field length.

    //Namespace associated with our SVG struct.  May not be empty.  While a real SVG struct might have
    ...
    //Description of our SVG struct - from the optional <desc> element.  May be empty.
...
/* For the four "num..." functions below, you need to search the SVG struct for components that match the search 
  criterion.  You may wish to write some sort of a generic searcher function that accepts a struct, a predicate function,
 ...
 *@post The appropriate attribute was set correctly
 *@return a boolean value indicating success or failure of the function
 *@param
    struct - a pointer to an SVG struct
    elemType - enum value indicating element to modify
    elemIndex - index of the element to modify
...
/** Function to adding an element - Circle, Rectangle, or Path - to an SVG
 *@pre
    SVG object exists, is valid, and and is not NULL.
 ...
 *@param
    struct - a pointer to an SVG struct
    elemType - enum value indicating element to modify
    newElement - pointer to the element struct (Circle, Rectangle, or Path)
 **/
